...
class SubscribeSMSTests(TestCase):
    ...
    def test_valid_subscribe(self):
        ...
        self.add_sms_user.delay.assert_called_with(
            "SMS_Android", "+19198675309", False, vendor_id="the dude",
        )
    ...
    def test_valid_non_us_subscribe_default_lang(self):
        ...
        self.add_sms_user.delay.assert_called_with(
            "SMS_Android", "+918088675309", False, vendor_id="the dude",
        )
    ...
    def test_valid_non_us_subscribe(self):
        ...
        self.add_sms_user.delay.assert_called_with(
            "SMS_Android", "+449198675309", False, vendor_id="sir dude",
        )
        # with country code
        ...
        self.add_sms_user.delay.assert_called_with(
            "SMS_Android", "+499198675309", False, vendor_id="herr dude",
        )
...
class FxAConcertsRSVPTests(ViewsPatcherMixin, TestCase):
    ...
    def test_valid_submission(self):
        ...
        self.record_fxa_concerts_rsvp.delay.assert_called_with(
            email="dude@example.com", is_firefox="Y", campaign_id="bowling",
        )
        self.assertEqual(resp.status_code, 200, resp.content)
        data = json.loads(resp.content)
        self.assertEqual("ok", data["status"])
    ...
    def test_requires_all_fields(self):
        self.has_valid_api_key.return_value = True
        req = self.rf.post("/", {"email": "dude@example.com", "is_firefox": "yes"})
        resp = views.fxa_concerts_rsvp(req)
        self.assertFalse(self.record_fxa_concerts_rsvp.delay.called)
        self.assertEqual(resp.status_code, 401, resp.content)
        data = json.loads(resp.content)
...
class SubHubAPITests(ViewsPatcherMixin, TestCase):
    def setUp(self):
        self.client = Client()
